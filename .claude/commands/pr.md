...
3. **ブランチ決定**:
   - 現在のブランチが main の場合: 変更内容に基づいて適切なブランチ名を自分で決定し、新しいブランチを作成してチェックアウトする（例: `feature/add-dark-mode`, `fix/login-error`, `refactor/auth-logic`）
...
8. **PR作成**: `gh pr create --base main` でPRを作成する。ベースブランチは必ず **main** を指定する。PR のタイトルと説明は日本語で作成する。
